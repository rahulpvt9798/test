...
# Simplified FFmpeg command for multi-quality HLS
ffmpeg -allowed_extensions ALL \
  -decryption_key "$KEY" \
  -i "$INPUT" \
  -map 0:v:0 -c:v:0 libx264 -b:v:0 500k  -s:v:0 426x240 -r:v:0 50 \
  -map 0:v:0 -c:v:1 libx264 -b:v:1 800k  -s:v:1 640x360 -r:v:1 50 \
  -map 0:v:0 -c:v:2 libx264 -b:v:2 1200k -s:v:2 854x480 -r:v:2 50 \
  -map 0:v:0 -c:v:3 libx264 -b:v:3 2500k -s:v:3 1280x720 -r:v:3 50 \
  -map 0:v:0 -c:v:4 libx264 -b:v:4 5000k -s:v:4 1920x1080 -r:v:4 50 \
  ...
  -hls_segment_filename "/app/hls/%v_%03d.ts" \
  /app/hls/%v.m3u8
